...
static void app_wifi_print_qr(const char *name, const char *pop, const char *transport)
{
    ...
    esp_qrcode_config_t qr_cfg = {
        .display_func        = esp_qrcode_print_console,   // in QR ra console
        .max_qrcode_version  = 10,                         // tuỳ bạn: 5–10 là ổn
        .qrcode_ecc_level    = ESP_QRCODE_ECC_LOW          // cùng mức ECC như code cũ
    };
    esp_err_t er = esp_qrcode_generate(&qr_cfg, payload);
    if (er != ESP_OK) {
        ESP_LOGW(TAG, "QR generate failed: %s", esp_err_to_name(er));
    }
...
esp_err_t app_wifi_start(app_wifi_pop_type_t pop_type)
{
    ...
    network_prov_mgr_config_t config = {
         ...
         * NETWORK_PROV_EVENT_HANDLER_NONE when using wifi_prov_scheme_softap*/
#ifdef CONFIG_APP_WIFI_PROV_TRANSPORT_BLE
